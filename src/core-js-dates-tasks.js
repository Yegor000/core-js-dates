...
function getNextFriday(date) {
  const d = new Date(date);
  d.setDate(d.getDate() + ((7 + 5 - d.getDay()) % 7 || 7));
  return d;
}
...
function formatDate(date) {
  ...
  const period = d.getUTCHours() > 11 ? 'PM' : 'AM';
...
function getWeekNumberByDate(date) {
  ...
  const firstWeek = firstDay.getDay() > 4 ? 7 - firstDay.getDay() : 0;
  const oneDay = 1000 * 60 * 60 * 24;
  const currentDay = (d - firstDay - firstWeek) / oneDay;
...
function getQuarter(date) {
  ...
  const oneDay = 1000 * 60 * 60 * 24;
  const allDays = 365 + getCountDaysInMonth(2, d.getFullYear()) - 28;

  return Math.floor((d - firstDay) / oneDay / (allDays / 4)) + 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (
    let i = 0;
    i < getCountDaysOnPeriod(startDate, endDate);
    i += countWorkDays + countOffDays
  ) {
    for (let j = 0; j < countWorkDays; j += 1) {
      if (i + j >= getCountDaysOnPeriod(startDate, endDate)) {
        break;
      }

      const currentDate = new Date(startDate.getTime() + oneDay * (i + j));
      schedule.push(
